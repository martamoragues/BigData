...
do
	...
	do
        ...
        ${HADOOP_HOME}/bin/hadoop --config $CONF jar /scratch/nas/2/martam/hadoop-0.20.203.0/hadoop-examples-0.20.203.0.jar wordcount -D sampling.estrategia=7 $INPUT $OUTPUT/E0/IT_$ITERATION/$(printf %03d $PERCENT)

        echo "Copying results of iteration $ITERATION with percent $PERCENT to local hd"
        ${HADOOP_HOME}/bin/hadoop --config $CONF fs -getmerge $OUTPUT/E0/IT_$ITERATION/$(printf %03d $PERCENT) $LOCAL_HD/E0/IT_$ITERATION/E0_$(printf %03d $PERCENT).txt
        ...
        ssh -p 12025 -oStrictHostKeyChecking=no -i /scratch/nas/2/martam/ssh/id_dsa jcugat@127.0.0.1 "mkdir -p /media/jcugat/marta/core/wc/E0/IT_$ITERATION/"
        rsync -az -e "ssh -p 12025 -oStrictHostKeyChecking=no -i /scratch/nas/2/martam/ssh/id_dsa" $LOCAL_HD/E0/IT_$ITERATION/E0_$(printf %03d $PERCENT).txt jcugat@127.0.0.1:/media/jcugat/marta/core/wc/E0/IT_$ITERATION/E0_$(printf %03d $PERCENT).txt
        ...
        rm -rf $LOCAL_HD/E0/IT_$ITERATION/E0_$(printf %03d $PERCENT).txt
