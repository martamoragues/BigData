...
do
    ...
    do
        echo "Executing iteration $ITERATION with percent $PERCENT"
   		${HADOOP_HOME}/bin/hadoop --config $CONF jar /scratch/nas/2/martam/hadoop-0.20.203.0/hadoop-examples-0.20.203.0.jar sort  -D sampling.seed=2 -D sampling.estrategia=2 -D sampling.P=$PERCENT \
            $COMPRESS_OPT \
...
do
    ...
    do
        ...
        ${HADOOP_HOME}/bin/hadoop --config $CONF fs -getmerge $OUTPUT/E2/IT_$ITERATION/$(printf %03d $PERCENT) $RESULT/E2/IT_$ITERATION/E2_$(printf %03d $PERCENT).txt
    done
done
