...
do
    ...
    do
        echo "Executing iteration $ITERATION with percent $PERCENT"
   		${HADOOP_HOME}/bin/hadoop --config $CONF jar /scratch/nas/2/martam/hadoop-0.20.203.0/hadoop-examples-0.20.203.0.jar sort  -D sampling.all.file.size=3544058682 -D sampling.estrategia=3 -D sampling.P=${TRADUCCIO_PERCENT[$PERCENT]} \
            $COMPRESS_OPT \
