...
do
        ...
        ${HADOOP_HOME}/bin/hadoop --config $CONF jar /scratch/nas/2/martam/hadoop-0.20.203.0/hadoop-examples-0.20.203.0.jar wordcount -D sampling.seed=2 -D sampling.estrategia=1 -D sampling.P=50 $INPUT $OUTPUT/E1/IT_$ITERATION/
...
for ITERATION in {1..1}
do
        echo "Copying results of iteration $ITERATION"
        ${HADOOP_HOME}/bin/hadoop --config $CONF fs -getmerge $OUTPUT/E1/IT_$ITERATION/ $RESULT/E1/IT_$ITERATION/E1.txt
done
